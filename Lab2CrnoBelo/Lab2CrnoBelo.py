...
class CrnoBelo(Problem):
    def __init__(self, initial):
        self.initial = initial
        self.goal = tuple([1 for _ in range(n*n)])
        print(self.goal)
    ...
    def successor(self, state):
        ...
        for i in range(n):
            for j in range(n):
                mat = [[state[m] for m in range(n * x, n * (x + 1))] for x in range(n)]
                mat[i][j] = invert(mat[i][j])
                if i + 1 < n:
                    mat[i + 1][j] = invert(mat[i + 1][j])
                if i - 1 >= 0:
                    mat[i - 1][j] = invert(mat[i - 1][j])
                if j + 1 < n:
                    mat[i][j + 1] = invert(mat[i][j + 1])
                if j - 1 >= 0:
                    mat[i][j - 1] = invert(mat[i][j - 1])
                f = []
                for li in mat:
...
crnoBeloinstance = CrnoBelo(tup)
answer = breadth_first_graph_search(crnoBeloinstance)
